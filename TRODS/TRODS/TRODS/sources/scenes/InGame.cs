...
namespace TRODS
{
    ...
    class InGame : AbstractScene
    {
        ...
        public InGame(Rectangle windowSize, KeyboardState keyboardState, MouseState mouseState)
        {
            ...
            personnage = new Sprite(new Rectangle(_windowSize.Width / 2 - 50, 2 * _windowSize.Height / 3, 100, 100), _windowSize, "game/bear");

            mouse = new Sprite(new Rectangle(-100, -100, 30, 50), _windowSize);
            map = new AbstractMap(_windowSize);
            map.VuePosition = new Vector2(_windowSize.Width / 2, _windowSize.Height / 2 + 49);
            map.Visitable.Add(new Rectangle(_windowSize.Width / 2, _windowSize.Height / 2, 1200, 50));
            map.Elements.Add(new AbstractMap.Element(new Sprite(new Rectangle(0, 0, _windowSize.Width, _windowSize.Height), _windowSize, "map1/sky1"), 0.2f, 0, true));
            map.Elements.Add(new AbstractMap.Element(new Sprite(new Rectangle(0, 0, _windowSize.Width, _windowSize.Height), _windowSize, "map1/back1r"), 0.8f, 0.2f, true));
            map.Elements.Add(new AbstractMap.Element(new Sprite(new Rectangle(0, _windowSize.Height / 2, _windowSize.Width, _windowSize.Height / 2), _windowSize, "map1/fore1"), 1f, 0.5f, true));
        }
